...
void MarkCompact::mark_step()
{
    ...
    for (iterator = used_table->begin(); iterator != used_table->end(); iterator++) {
        ...
        if (cell->get_size() == 4 && !cell->get_transer_object()) {
            INFO_PRINT("\t 打上标记\n");

            /* 将打标记的对象放入transer_table中，这个对象不能释放，释放了就无法实现内存移动，旧地址仍旧可以访问的功能 */
            get_mem_chunk()->get_transer_table()->push_front(cell);

            /* 当前iterator重新设置为放入transer_table中对象的克隆，为了防止后续操作将旧地址指向的对象释放了 */
...
void MarkCompact::data_compact_step()
{
    ...
    list<MemoryCell *>::iterator transer_ite = get_mem_chunk()->get_transer_table()->begin();

    while (transer_ite != get_mem_chunk()->get_transer_table()->end()) {
        MemoryCell *transer_cell = *transer_ite;

        transer_cell->to_string("开始整理数据");

        get_mem_chunk()->malloc_after_gc(transer_cell);

        transer_ite = get_mem_chunk()->get_transer_table()->erase(transer_ite);
    }
