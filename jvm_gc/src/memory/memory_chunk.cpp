...
list<MemoryCell *> *MemoryChunk::get_transer_table()
{
    return &m_transer_table;
}
...
MemoryCell *MemoryChunk::malloc_after_gc(MemoryCell *transer_cell)
{
    MemoryCell *ret = NULL;
    ...
    for (available_iterator = m_available_table.begin(); available_iterator != m_available_table.end(); available_iterator++) {
        ...
        if (cell->get_size() >= transer_cell->get_size()) {
            cell->to_string("找到了满足条件的Cell");

            /* 原地址：数据的位置 */
            ptr_from = transer_cell->ptr();

            /* 调整transer cell后插入used table */
            transer_cell->set_start(cell->get_start());
            transer_cell->set_end(transer_cell->get_start() + transer_cell->get_size());

            /* 新地址，需要将数据拷贝过来 */
            ptr_to = transer_cell->ptr();

            INFO_PRINT("开始拷贝内存: ptr_from=%X, ptr_to=%X, size=%d\n", ptr_from, ptr_to, transer_cell->get_size() * get_align_size());

            memcpy(ptr_to, ptr_from, transer_cell->get_size() * get_align_size());

            m_used_table.push_front(transer_cell);

            /**
             * 处理Cell信息
             */
            cell->inc_start(transer_cell->get_size())->desc_size(transer_cell->get_size());

            /**
             * 更新used_cell_num
             */
            inc_used_cell_num(transer_cell->get_size());
...
void MemoryChunk::print_transer_table()
{
    PRINT("[打印transer_table]开始, size=%d\n", get_transer_table()->size());

    list<MemoryCell *>::iterator tmp;

    for (tmp = m_transer_table.begin(); tmp != m_transer_table.end(); tmp++) {
        PRINT("\t %X: cell_start=%d, cell_end=%d, cell_size=%d\n",
                *tmp, (*tmp)->get_start(), (*tmp)->get_end(), (*tmp)->get_size());
    }

    PRINT("[打印transer_table]结束\n");
}
...
void MemoryChunk::collect(size_t size) {
